-- ============================================================================
-- Atomic staff order placement with a client-supplied idempotency key — the
-- write path used by the offline queue (replayed on reconnect). Dedups on
-- orders.unique(tenant_id, idempotency_key): a replay returns the existing
-- order id and does not re-insert items. Role-gated (staff), audited by RLS.
-- ============================================================================
...
begin
  ...
  -- Replay of an already-created order → return it, items already present.
  ...
  if _table_id is not null then
    ...
    where id = _table_id and state = 'free';
  end if;
