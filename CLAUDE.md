...
- The same boundary **in the other direction is worse, because it builds fine**: a Server Component may not import a non-component *value* from a `"use client"` module. Every export of a client module becomes a *client reference* on the server, so calling a plain function throws `Attempted to call a temporary Client Reference from the server` — at render, not at build. `tsc` and `next build` both pass and the page 500s the first time anyone opens it. Shared values go in a plain module both sides import. `npm run check:rsc` fails the build on this.
